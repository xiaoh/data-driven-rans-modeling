        ...
        if fnmatch.fnmatch(file, 'pehill-tmp*'):
            ...
            if plotFlag == True:
                ...
                for i in range(len(filenames)):
                    ...
                    p1, = plt.plot(x, y,'-', color='grey', alpha=0.5, lw = 1, markevery=20,mfc='none')
...
def plotBaseline(scaleV, xCol, yCol):
    filename = 'pehill-observation/postProcessing/surfaces/'+timeDir + \
...
def main(iShow=False):
    ...
    p1, = plt.plot(scatSample[:,1],np.zeros_like(scatSample[:,1]), 'o', color='grey', markersize=8,\
    ...
    fig.savefig("bubble-"+caseName+".pdf")
