...
for numHill in range(0,1):
    for xPos in xPosList: for file in os.listdir('.'): if fnmatch.fnmatch(file, 'duct-tmp*'):
                plotFlag = False
                for sample in samplePlotList:
                    if fnmatch.fnmatch(file, '*_'+str(sample)+'.0*'):
                        plotFlag = True
                if plotFlag == True:
                    Mbase = np.loadtxt('duct-observation/postProcessing/sets/0/line_x' + str(xPos) + '_Tau.xy');
                    Msample = np.loadtxt(file + '/postProcessing/sets/'+timeDir+'/line_x' + str(xPos) + '_Tau.xy');
                    Mdns = np.loadtxt('DNS/Tau' + str(xPos));
                    ...
                    p1, p2, p3 = R0.plotTauOnBaycentric(tauBase,tauDNS,tauSample)
        lg = plt.legend([p1,p3,p2], legendList, loc = 0)
        lg.draw_frame(False)
        plt.savefig("figures/perturbComp" + str(xPos) + ".pdf")
        plt.clf()
