...
defineTypeNameAndDebug(myLRR, 0);
addToRunTimeSelectionTable(RASModel, myLRR, dictionary);
...
tmp<volSymmTensorField> myLRR::devReff() const
{
    ...
    (
        ...
        (
            ...
            Tau_ - nu()*dev(twoSymm(fvc::grad(U_)))
        )
    );
}


tmp<fvVectorMatrix> myLRR::divDevReff(volVectorField& U) const
{
    if (couplingFactor_.value() > 0.0)
    {
        return
        (
            fvc::div(Tau_ + couplingFactor_*nut_*fvc::grad(U), "div(R)")
          + fvc::laplacian
            (
...
tmp<fvVectorMatrix> myLRR::divDevRhoReff
(
...
) const
{
    ...
    if (couplingFactor_.value() > 0.0)
    {
        ...
        (
            fvc::div
            (
                rho*Tau_ + couplingFactor_*(rho*nut_)*fvc::grad(U),
                "div((rho*R))"
            )
          + fvc::laplacian
            (
...
void myLRR::correct()
{
    ...
    volSymmTensorField P(-twoSymm(Tau_ & fvc::grad(U_)));
    ...
    //    fvm::ddt(Tau_)
    //  + fvm::div(phi_, Tau_)
    ////- fvm::laplacian(Cs*(k_/epsilon_)*Tau_, Tau_)
    //  - fvm::laplacian(DREff(), Tau_)
    //  + fvm::Sp(Clrr1_*epsilon_/k_, Tau_)
    //  ==
    //    P
    ...
    forAll(patches, patchi)
    {
        ...
        if (isA<wallFvPatch>(curPatch))
        {
            symmTensorField& Rw = Tau_.boundaryField()[patchi];
